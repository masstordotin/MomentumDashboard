...
### Running during live market hours

Source #2 above (Yahoo's short-range daily fetch) can include **today's still-forming intraday bar** whenever the screener is run while NSE is open — its close is just the latest traded price (still changing) and its volume is only however much has traded so far that session, nowhere near a full day's total. Scoring against that as if it were a completed session previously caused the screener to reject almost everything: partial-day volume divided into a 30-day average of *full*-day volumes fails the `volRatio >= 1.5` gate for nearly every stock, and exactly which ones slip through becomes a function of what time of day the script happened to run — which is why two runs described as "the same criteria" could produce wildly different candidate counts (including zero).

`screen_one()` now calls `_drop_unconfirmed_today()` before scoring: any candle dated *today* (IST) is dropped from the merged series unless NSE's own bhavcopy has already published that exact date. Since bhavcopy is only ever generated after a session settles, its presence is proof the day is genuinely complete — not a live snapshot. In practice this means: running the screener mid-session correctly scores against the last *completed* trading day, and running it after bhavcopy for today has been published (typically evening IST) correctly picks up today's real close.
